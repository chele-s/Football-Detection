"""
GPU-Accelerated Stream Pipeline
================================

Zero-copy pipeline: Video frames stay in VRAM throughout entire pipeline
- NVDEC (decode) → PyTorch tensors (crop/resize) → RF-DETR (inference) → NVENC (encode)

Performance: 3-5x faster than CPU pipeline, frees CPU for tracking logic
"""
...
from app.utils import RTMPClient, GPU_AVAILABLE

if GPU_AVAILABLE:
    from app.utils import GPUVideoReader, GPUVideoWriter, GPUTensorOps

logger = logging.getLogger(__name__)


class SmoothZoom:
    """Smooth zoom controller (same as original)"""
    def __init__(self, min_zoom: float = 1.0, max_zoom: float = 2.5, 
                 stiffness: float = 0.08, damping: float = 0.70, max_rate: float = 0.08):
...
class GPUStreamPipeline:    
    def __init__(self, config: Dict[str, Any]):
        if not GPU_AVAILABLE:
            raise RuntimeError(
                "GPU pipeline requires PyNvCodec. Install with:\n"
                "  python install_pynvcodec_colab.py  # For Colab\n"
                "  bash install_pynvcodec.sh          # For local Linux\n"
            )
        
        if not torch.cuda.is_available():
            raise RuntimeError("GPU pipeline requires CUDA-capable GPU")
        ...
        logger.info("="*70)
        logger.info("🚀 GPU-ACCELERATED STREAM PIPELINE")
        logger.info("="*70)
        
        if not self.display_available:
            logger.info("No display detected - running headless (cv2.imshow disabled)")
        
        # Initialize detector (already GPU-optimized)
        ...
        logger.info(f"✓ RF-DETR loaded: {model_info['model_type']} on {model_info['device']}")
        
        # Initialize tracker (CPU, lightweight)
    ...
    def run(self, input_source: str, output_destination: Optional[str] = None):
        ...
        # Initialize GPU video reader (NVDEC)
        logger.info(f"Initializing GPU decoder (NVDEC) for: {input_source}")
        reader = GPUVideoReader(input_source, device=0)
        ...
        # Initialize GPU video writer (NVENC)
        writer = None
        if output_destination and not self.debug_mode:
            logger.info(f"Initializing GPU encoder (NVENC) for: {output_destination}")
            writer = GPUVideoWriter(
                output_url=output_destination,
                width=self.config['output']['width'],
                height=self.config['output']['height'],
                fps=self.target_fps,
                bitrate=int(self.config['stream'].get('bitrate', '4000k').replace('k', '000')),
                preset=self.config['stream'].get('preset', 'ultrafast'),
                device=0
            )
            logger.info("✓ GPU encoder ready")
        ...
        logger.info("🎬 Starting GPU pipeline... (Press 'q' to quit)")
        logger.info("="*70)
        ...
        try:
            while True:
                ...
                self.performance_stats['decode_times'].append(t_decode)
                
                if not ret:
                    logger.info("End of stream")
                    break
                
                # frame_tensor is now [3, H, W] in VRAM, RGB, float [0..1]
                ...
                if prev_crop is not None and roi_active:
                    ...
                    det_result = self.detector.predict_ball_only(
                        frame_roi, self.ball_class_id, 
                        use_temporal_filtering=False, return_candidates=True
                    )
                else:
                    det_result = self.detector.predict_ball_only(
                        frame_tensor, self.ball_class_id, return_candidates=True
                    )
                ...
                # Define dead zones (same as original pipeline)
                dead_zones = [
                    {'x1': w * 0.65, 'y1': 0, 'x2': w, 'y2': h * 0.45, 'name': 'top-right'},
                    {'x1': 0, 'y1': 0, 'x2': w * 0.08, 'y2': h * 0.40, 'name': 'top-left'},
                ]
                ...
                if use_roi and detection is not None:
                ...
                if use_roi and detections_list:
                    mapped = []
                    for d in detections_list:
                        global_x = d[0] + offx
                        global_y = d[1] + offy
                ...
                if apply_spatial_filter:
                    if detection is not None:
                        bx, by, bw, bh, bconf = detection
                        if by < h * 0.20 or bx < w * 0.10 or bx > w * 0.90:
                            detection = None
                    ...
                    if detections_list:
                        ...
                        for d in detections_list:
                            dx, dy = d[0], d[1]
                            if dy >= h * 0.20 and dx >= w * 0.10 and dx <= w * 0.90:
                                filtered_dets.append(d)
                        detections_list = filtered_dets if filtered_dets else None
                
                # Reject ambiguous detections
                if detections_list and len(detections_list) >= 4:
                    if detection is not None:
                        ...
                        if len(other_high_conf) >= 2:
                            logger.warning(f"Multiple candidates - REJECTING")
                            detection = None
                            detections_list = None
                ...
                if not camera_initialized and track_result:
                    ...
                    crop_coords = self.virtual_camera.update(
                        x, y, time.time(), 
                        velocity_hint=self.tracker.get_velocity(), 
                    ...
                    if state and 'stats' in state:
                        ...
                        if total > 0:
                            detector_stable = erratic / total < 0.08
                    ...
                    safe_x = max(crop_half_w + 20, min(use_x, w - crop_half_w - 20))
                    safe_y = max(crop_half_h + 20, min(use_y, h - crop_half_h - 20))
                    
                    if not is_tracking:
                        prev_y = self.virtual_camera.current_center_y
                        min_allowed_y = int(h * 0.35)
                        safe_y = max(min_allowed_y, prev_y)
                    
                    vhx, vhy = self.tracker.get_velocity()
                    crop_coords = self.virtual_camera.update(
                        safe_x, safe_y, time.time(), 
                    ...
                    search_target_x = int(w * 0.50)
                    search_target_y = int(h * 0.40)
                    ...
                    crop_coords = self.virtual_camera.update(
                        lost_search_center[0], lost_search_center[1], 
                        time.time(), velocity_hint=(0, 0), detector_stable=True
                    )
                ...
                # ===== CROP & RESIZE ON GPU =====
                output_h = self.config['output']['height']
                output_w = self.config['output']['width']
                ...
                # cropped_tensor is now [3, output_h, output_w] in VRAM
                
                # ===== OVERLAY STATS (optional, requires CPU copy) =====
                if self.show_stats:
                    import cv2
                    # Convert to CPU numpy only for stats overlay
                    ...
                    avg_decode = np.mean(self.performance_stats['decode_times']) if self.performance_stats['decode_times'] else 0
                    ...
                    cv2.putText(display_frame, f"FPS: {avg_fps:.1f} [GPU]", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                    cv2.putText(display_frame, f"Status: {status}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                    cv2.putText(display_frame, f"Decode: {avg_decode:.1f}ms", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
                    cv2.putText(display_frame, f"Inference: {avg_inference:.1f}ms", (10, 115), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
                    cv2.putText(display_frame, f"Tracking: {avg_tracking:.1f}ms", (10, 140), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
                    cv2.putText(display_frame, f"Camera: {avg_camera:.1f}ms", (10, 165), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
                    
                    if track_result:
                        tracker_state = self.tracker.get_state()
                        vel_mag = tracker_state['velocity_magnitude']
                        cv2.putText(display_frame, f"Velocity: {vel_mag:.1f}px/s", (10, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
                        cv2.putText(display_frame, f"Conf: {tracker_state['avg_confidence']:.2f}", (10, 215), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)
                    
                    # Convert back to GPU tensor for encoding
                    # BGR → RGB, [H,W,3] → [3,H,W], [0..255] → [0..1]
                    display_frame_rgb = display_frame[:, :, ::-1].copy()
                    cropped_tensor = torch.from_numpy(display_frame_rgb).permute(2, 0, 1).float() / 255.0
                    cropped_tensor = cropped_tensor.cuda(self.device, non_blocking=True)
                
                # ===== ENCODE ON GPU (NVENC) =====
                if writer:
                    t_encode_start = time.time()
                    writer.write(cropped_tensor)
                    t_encode = (time.time() - t_encode_start) * 1000
                    self.performance_stats['encode_times'].append(t_encode)
                ...
                if self.debug_mode and self.display_available:
                    ...
                    if not self.show_stats:
                        display_frame = GPUTensorOps.to_numpy_cpu(cropped_tensor)
                    cv2.imshow('GPU Stream Output', display_frame)
                ...
                if frame_count % 100 == 0:
                    ...
                    logger.info(f"[GPU] Frames: {frame_count} | FPS: {fps:.2f} | "
    ...
    def _log_final_stats(self, total_frames: int):
        ...
        logger.info("="*70)
        logger.info("FINAL STATISTICS (GPU PIPELINE)")
        logger.info("="*70)
        logger.info(f"Total frames processed: {total_frames}")
        
        if self.performance_stats['decode_times']:
            avg = np.mean(self.performance_stats['decode_times'])
            logger.info(f"Avg decode time (NVDEC): {avg:.2f}ms")
        
        if self.performance_stats['inference_times']:
            avg = np.mean(self.performance_stats['inference_times'])
            logger.info(f"Avg inference time: {avg:.2f}ms")
        
        if self.performance_stats['tracking_times']:
            avg = np.mean(self.performance_stats['tracking_times'])
            logger.info(f"Avg tracking time: {avg:.2f}ms")
        
        if self.performance_stats['camera_times']:
            avg = np.mean(self.performance_stats['camera_times'])
            logger.info(f"Avg camera time: {avg:.2f}ms")
        
        if self.performance_stats['encode_times']:
            avg = np.mean(self.performance_stats['encode_times'])
            logger.info(f"Avg encode time (NVENC): {avg:.2f}ms")
